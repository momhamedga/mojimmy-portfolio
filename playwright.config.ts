...
/**
 * عتبة المقارنة البصرية — صفر، ولا قيمة «نهائية» بعد.
 *
 * المعايرة جرت ولم تنجح، والصفر هنا موقف صريح لا إهمال: لا نترك في الإعداد
 * رقمًا يبدو معتمدًا وقد أسقطته الشهادة.
 *
 * ما ثبت بالقياس: الصور متطابقة بايتيًا على المُشغِّل الواحد داخل التشغيل
 * الواحد، لكن لقطة القائمة الزجاجية (`backdrop-filter`) تنفلت متقطّعةً بين
 * التشغيلات. المسح على الانزياح المرصود في 5B.2 أعطى حدًّا عند ٠٫٠٢٣
 * (٠٫٠٢٢ تسقط ببكسل واحد)، فثُبّتت ٠٫٠٢٥ — ثم أسقطتها الشهادة على ثلاثة
 * مُشغِّلات: مرّت على D وسقطت على B وC ببكسل واحد لا غير.
 *
 * حساب ذلك البكسل بمعادلة pixelmatch نفسها: (359,736) ينتقل من rgb(17,21,26)
 * إلى rgb(9,13,18)، أي delta=32.34، فيلزمه أكثر من ٠٫٠٣٠٣ — وميزانية ٠٫٠٣
 * هي ٣١٫٦٩ فقط. أي أن سقف السُلّم المعتمد لا يكفي أصلًا.
 *
 * والرفع ليس الحلّ الواضح: انفلات بكسل واحد عشوائي مع `maxDiffPixels: 0`
 * يعني أن أي عتبة عامة تلاحق هدفًا متحرّكًا. للمقارنة: إزاحة عنصر واحد ١px
 * تنتج ١٦٤ بكسل مختلفًا، فالفصل بين الضجيج والتغيّر الحقيقي واسع — وهو ما
 * يجعل تسامحًا انتقائيًا لتلك اللقطة وحدها اقتراحًا معقولًا، لكنه قرار يخصّ
 * صاحب المشروع لا هذا الملف.
 *
 * `maxDiffPixels` و`maxDiffPixelRatio` صفر ويبقيان: لا مسامحة بالعدد.
 * `PW_VISUAL_THRESHOLD` يبقى لإعادة المعايرة.
 */
const DEFAULT_VISUAL_THRESHOLD = 0;
